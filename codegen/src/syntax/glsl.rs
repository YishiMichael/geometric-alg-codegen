...
use crate::syntax::{Stringifier, Writer};
...
pub fn emit_record<A: Ast>(
    ...
    for implementation in &record.implementations {
        writer.newline()?;
        emit_implementation(writer, stringifier, implementation, false)?;
    }
    for implementation in &record.implementations {
        writer.newline()?;
        emit_implementation(writer, stringifier, implementation, true)?;
    }
    Ok(())
}
...
fn emit_structure<A: Ast>(
    ...
    )?;
    {
        ...
        if fields.is_empty() {
            writer.newline()?;
            write!(writer.buffer(), "int _phantom;")?;
        }
        writer.dedent();
    }
    writer.newline()?;
